...
# 9.1. 计算APK的MD5并更新download_url，同时生成按MD5命名的APK文件
echo "🔐 计算APK文件MD5并更新下载链接..."

# 使用Python脚本计算MD5（更稳定，支持大文件）
PYTHON_SCRIPT="$UPDATE_DIR/generate_file_info.py"
if [ ! -f "$PYTHON_SCRIPT" ]; then
    echo "[WARN] generate_file_info.py脚本不存在，跳过download_url和MD5命名APK更新"
else
    PYTHON_BIN="${PYTHON_BIN:-python}"

    APK_MD5=$("$PYTHON_BIN" "$PYTHON_SCRIPT" "$APK_PATH" --md5-only 2>/dev/null | tr -d ' \n\r')
    
    if [ -z "$APK_MD5" ] || [ ${#APK_MD5} -ne 32 ]; then
        echo "[WARN] 无法计算APK的MD5值，跳过download_url和MD5命名APK更新"
    else
        echo "   APK MD5: $APK_MD5"

        # 生成按 MD5 命名的 APK 文件（便于直接上传服务器）
        MD5_APK_NAME="${APK_MD5}.apk"
        MD5_APK_PATH="$UPDATE_DIR/$MD5_APK_NAME"
        cp "$APK_PATH" "$MD5_APK_PATH"
        echo "   已生成按MD5命名的APK: $MD5_APK_NAME"
        ...
        if [ -n "$CURRENT_DOWNLOAD_URL" ]; then
            ...
            if [ -n "$BASE_URL" ]; then
                ...
                echo "[WARN] 无法从现有download_url中提取基础URL，跳过download_url更新"
            fi
        else
            echo "[WARN] 无法读取现有的download_url，跳过download_url更新"
        fi
